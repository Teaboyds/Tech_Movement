...
type NewsRepository interface {
	...
	Find(catID, ConType, Sort, status, view, search string, limit, page int64) ([]*d.News, error)
...
type NewsService interface {
	...
	Delete(id string) error
	DeleteMany(id []string) error
	Find(catID, ConType, Sort, limit, page, status, view, search string) ([]*d.NewsResponseV2, error)
